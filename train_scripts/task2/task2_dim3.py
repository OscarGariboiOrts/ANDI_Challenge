...
def main(args):
    """
        ...
        Script to train model for classification of trajectories in 3D under the scope of the ANDI Challenge.
    ...
    for i in range(len(task_raw)):
        tercio = int(len(task_raw[i])//3)
        aux = []
        for j in range(tercio):
            aux.append((task_raw[i][j], task_raw[i][j+tercio], task_raw[i][j+(2*tercio)]))
    ...
    task_HomLenRes = task_HomLen.reshape((-1, 1000,3))    
    ...
    model = Sequential()
    model.add(Conv1D(filters=64, kernel_size=8, activation='relu', input_shape=(1000,3)))
    ...
    mc = ModelCheckpoint(path_to_model + 'task2_dim3-{epoch:03d}-{val_accuracy:.4f}.h5', monitor='val_accuracy', mode='max', verbose=0, save_best_only=True)
